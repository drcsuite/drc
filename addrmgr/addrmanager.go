...
// AddrManager提供了一个并发安全的地址管理器，用于缓存比特币网络上的潜在对等点。
...
// 地址spriority type用于描述本地地址发现方法的层次结构。
...
// AddAddress向地址管理器添加一个新地址。它强制执行最大数量的地址，并无声地忽略重复的地址。它对于并发访问是安全的。
...
// AddAddressByIP添加一个地址，在这个地址中我们被赋予一个ip:port，而不是一个wire.NetAddress。
