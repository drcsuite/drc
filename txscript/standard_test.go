...
// newAddressPubKey returns a new drcutil.AddressPubKey from the provided
...
func newAddressPubKey(serializedPubKey []byte) drcutil.Address {
	addr, err := drcutil.NewAddressPubKey(serializedPubKey,
...
// newAddressPubKeyHash returns a new drcutil.AddressPubKeyHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressPubKeyHash(pkHash []byte) drcutil.Address {
	addr, err := drcutil.NewAddressPubKeyHash(pkHash, &chaincfg.MainNetParams)
	if err != nil {
...
// newAddressScriptHash returns a new drcutil.AddressScriptHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressScriptHash(scriptHash []byte) drcutil.Address {
	addr, err := drcutil.NewAddressScriptHashFromHash(scriptHash,
		&chaincfg.MainNetParams)
	if err != nil {
...
// bogusAddress implements the drcutil.Address interface so the tests can ensure
...
// IsForNet lies blatantly to satisfy the drcutil.Address interface.
...
func TestPayToAddrScript(t *testing.T) {
	...
	p2pkhMain, err := drcutil.NewAddressPubKeyHash(hexToBytes("e34cce70c86"+
	...
	p2shMain, _ := drcutil.NewAddressScriptHashFromHash(hexToBytes("e8c300"+
	...
	p2pkCompressedMain, err := drcutil.NewAddressPubKey(hexToBytes("02192d"+
	...
	p2pkCompressed2Main, err := drcutil.NewAddressPubKey(hexToBytes("03b0b"+
	...
	p2pkUncompressedMain, err := drcutil.NewAddressPubKey(hexToBytes("0411"+
	...
	tests := []struct {
		...
		{(*drcutil.AddressPubKeyHash)(nil), "", errUnsupportedAddress},
		{(*drcutil.AddressScriptHash)(nil), "", errUnsupportedAddress},
		{(*drcutil.AddressPubKey)(nil), "", errUnsupportedAddress},
...
func TestMultiSigScript(t *testing.T) {
	...
	tests := []struct {
		keys      []*drcutil.AddressPubKey
		nrequired int
		expected  string
		err       error
	}{
		{
			[]*drcutil.AddressPubKey{
				p2pkCompressedMain,
				p2pkCompressed2Main,
			},
			1,
