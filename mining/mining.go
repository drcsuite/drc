...
const (
	...
	MinHighPriority = drcutil.SatoshiPerBitcoin * 144.0 / 250
...
func createCoinbaseTx(params *chaincfg.Params, coinbaseScript []byte, nextBlockHeight int32, addr drcutil.Address) (*drcutil.Tx, error) {
...
func spendTransaction(utxoView *blockchain.UtxoViewpoint, tx *drcutil.Tx, height int32) error {
...
func logSkippedDeps(tx *drcutil.Tx, deps map[chainhash.Hash]*txPrioItem) {
	if deps == nil {
		return
	}
...
//  |  <= policy.BlockMinSize)          |   |
//   -----------------------------------  --
func (g *BlkTmplGenerator) NewBlockTemplate(payToAddress drcutil.Address, pubkey *chainhash.Hash33) (*BlockTemplate, error) {
	...
	// avoided.
	blockTxns := make([]*drcutil.Tx, 0, len(sourceTxns))
...
func (g *BlkTmplGenerator) UpdateExtraNonce(msgBlock *wire.MsgBlock, blockHeight int32, extraNonce uint64) error {
	...
	// TODO(davec): A drcutil.Block should use saved in the state to avoid
