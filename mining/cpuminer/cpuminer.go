...
type Config struct {
	...
	ProcessBlock func(*drcutil.Block, blockchain.BehaviorFlags) (bool, error)
...
func (m *CPUMiner) submitBlock(block *drcutil.Block) bool {
	...
	log.Infof("Block submitted via CPU miner accepted (hash %s, "+
		"amount %v)", block.Hash(), drcutil.Amount(coinbaseTx.Value))
	return true
}
...
func (m *CPUMiner) solveBlock(msgBlock *wire.MsgCandidate, quit chan struct{}) bool {
