...
func TestFilterInsertKey(t *testing.T) {
	...
	wif, err := drcutil.DecodeWIF(secret)
	if err != nil {
	...
	f.Add(drcutil.Hash160(wif.SerializePubKey()))
...
func TestFilterBloomMatch(t *testing.T) {
	...
	tx, err := drcutil.NewTxFromBytes(strBytes)
	if err != nil {
	...
	spendingTx, err := drcutil.NewTxFromBytes(spendingTxBytes)
	if err != nil {
		t.Errorf("TestFilterBloomMatch NewTxFromBytes failure: %v", err)
		return
	}
