...
func (cs *CoinSet) TotalValue() (value drcutil.Amount) {
...
func satisfiesTargetValue(targetValue, minChange, totalValue drcutil.Amount) bool {
...
type CoinSelector interface {
	CoinSelect(targetValue drcutil.Amount, coins []Coin) (Coins, error)
}
...
type MinIndexCoinSelector struct {
	MaxInputs       int
	MinChangeAmount drcutil.Amount
}

// CoinSelect will attempt to select coins using the algorithm described
// in the MinIndexCoinSelector struct.
func (s MinIndexCoinSelector) CoinSelect(targetValue drcutil.Amount, coins []Coin) (Coins, error) {
	cs := NewCoinSet(nil)
...
func (s MinNumberCoinSelector) CoinSelect(targetValue drcutil.Amount, coins []Coin) (Coins, error) {
...
func (s MaxValueAgeCoinSelector) CoinSelect(targetValue drcutil.Amount, coins []Coin) (Coins, error) {
	sortedCoins := make([]Coin, 0, len(coins))
	sortedCoins = append(sortedCoins, coins...)
...
func (s MinPriorityCoinSelector) CoinSelect(targetValue drcutil.Amount, coins []Coin) (Coins, error) {
...
// drcutil.Tx, a specific outpoint index, and the number of confirmations
...
func (c *SimpleCoin) Value() drcutil.Amount {
	return drcutil.Amount(c.txOut().Value)
}
