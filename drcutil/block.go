...
// Block定义了一个比特币块，它提供了对原始块更容易和更有效的操作。
// 它还在第一次访问时为块及其事务缓存散列，这样后续访问就不必重复相对昂贵的散列操作。
...
// Tx returns a wrapped transaction (drcutil.Tx) for the transaction at the
...
// Transactions returns a slice of wrapped transactions (drcutil.Tx) for all
...
// instead provides easy access to wrapped versions (drcutil.Tx) of them.
