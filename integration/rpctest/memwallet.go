...
type memWallet struct {
	...
	addrs map[uint32]drcutil.Address
...
func (m *memWallet) IngestBlock(height int32, header *wire.BlockHeader, filteredTxns []*drcutil.Tx) {
...
func (m *memWallet) evalOutputs(outputs []*wire.TxOut, txHash *chainhash.Hash,
	...
	for i, output := range outputs {
		...
		for keyIndex, addr := range m.addrs {
			...
			m.utxos[op] = &utxo{
				value:          drcutil.Amount(output.Value),
				keyIndex:       keyIndex,
...
//
// This function is safe for concurrent access.
func (m *memWallet) ConfirmedBalance() drcutil.Amount {
	m.RLock()
	defer m.RUnlock()

	var balance drcutil.Amount
