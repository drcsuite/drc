...
//MedianTimeSource提供了一种添加多个时间样本的机制，这些时间样本用于确定中值时间，中值时间随后用作本地时钟的偏移量。
...
// AdjustedTime返回当前时间，该时间由AddTimeSample添加的时间样本计算的中值时间偏移量调整。
...
// AddTimeSample添加一个时间样本，用于确定添加的样本的中值时间。
...
// Offset返回根据AddTimeData添加的时间样本的中值调整本地时钟的秒数。
...
// NewMedianTime返回MedianTimeSource接口的并发安全实现的新实例。
