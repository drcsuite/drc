...
// IsSpent根据从未使用事务输出视图获得的输出的当前状态返回输出是否已使用。
...
func (view *UtxoViewpoint) AddTxOut(tx *drcutil.Tx, txOutIdx uint32, blockHeight int32) {
...
func (view *UtxoViewpoint) AddTxOuts(tx *drcutil.Tx, blockHeight int32) {
...
//connectTransaction通过添加由传递的事务创建的所有新utxos并将事务使用的所有utxos标记为已使用，从而更新视图。
// connectTransaction updates the view by adding all new utxos created by the
// passed transaction and marking all utxos that the transactions spend as
// spent.  In addition, when the 'stxos' argument is not nil, it will be updated
// to append an entry for each spent txout.  An error will be returned if the
// view does not contain the required utxos.
func (view *UtxoViewpoint) connectTransaction(tx *drcutil.Tx, blockHeight int32, stxos *[]SpentTxOut) error {
...
func (view *UtxoViewpoint) connectTransactions(block *drcutil.Block, stxos *[]SpentTxOut) error {
...
func (view *UtxoViewpoint) disconnectTransactions(db database.DB, block *drcutil.Block, stxos []SpentTxOut) error {
...
// fetchInputUtxos根据需要将给定块中事务引用的输入的未使用事务输出从数据库加载到视图中。
// fetchInputUtxos loads the unspent transaction outputs for the inputs
// referenced by the transactions in the given block into the view from the
// database as needed.  In particular, referenced entries that are earlier in
// the block are added to the view and entries that are already in the view are
// not modified.
func (view *UtxoViewpoint) fetchInputUtxos(db database.DB, block *drcutil.Block) error {
...
// FetchUtxoView从主链的末端为传递的事务引用的输入加载未使用的事务输出。
...
func (b *BlockChain) FetchUtxoView(tx *drcutil.Tx) (*UtxoViewpoint, error) {
...
// FetchUtxoEntry从主链的末端加载并返回请求的未使用事务输出。
