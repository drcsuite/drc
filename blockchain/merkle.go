...
// nextpowerof2从一个给定的数中返回2的下一个最高的幂，如果它还不是2的幂的话。
...
// HashMerkleBranches接受两个散列，作为左右树节点，并返回它们连接的散列。
...
// BuildMerkleTreeStore从事务片创建一个merkle树，
...
// is the zeroHash.
func BuildMerkleTreeStore(transactions []*drcutil.Tx, witness bool) []*chainhash.Hash {
...
// ExtractWitnessCommitment尝试定位并返回一个block的witness commitment。证明
...
func ExtractWitnessCommitment(tx *drcutil.Tx) ([]byte, bool) {
...
// validatewitness承诺验证在传递的块的coinbase事务中找到的witness承诺(如果有)。
...
func ValidateWitnessCommitment(blk *drcutil.Block) error {
