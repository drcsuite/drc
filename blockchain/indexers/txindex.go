...
//   Total: 44 bytes
// -----------------------------------------------------------------------------
// dbPutBlockIDIndexEntry使用现有的数据库事务更新或将哈希的索引项添加到id，并将id添加到提供值的哈希映射。
...
func dbAddTxIndexEntries(dbTx database.Tx, block *drcutil.Block, blockID uint32) error {
...
func dbRemoveTxIndexEntries(dbTx database.Tx, block *drcutil.Block) error {
...
//
// This is part of the Indexer interface.
func (idx *TxIndex) ConnectBlock(dbTx database.Tx, block *drcutil.Block,
...
//
// This is part of the Indexer interface.
func (idx *TxIndex) DisconnectBlock(dbTx database.Tx, block *drcutil.Block,
	stxos []blockchain.SpentTxOut) error {
