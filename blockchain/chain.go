...
// IsKnownOrphan返回传递的散列当前是否是已知的孤儿。请记住，只有有限数量的孤儿块被保留了有限的时间，
// 因此这个函数不能用作测试一个块是否是孤儿块的绝对方法。为此，必须将一个完整的块(而不仅仅是它的散列)传递给ProcessBlock。
// 但是，使用一个孤儿调用ProcessBlock会导致一个错误，因此这个函数为调用者提供了一种机制来智能地检测“最近的”重复孤儿，并作出相应的反应。
...
// remove孤儿院块从孤儿池和以前的孤儿索引中删除传递的孤儿块
...
// addOrphanBlock将传递的块(在调用此函数之前已确定为孤儿块)添加到孤儿池。
...
//
// This function is safe for concurrent access.
func (b *BlockChain) CalcSequenceLock(tx *drcutil.Tx, utxoView *UtxoViewpoint, mempool bool) (*SequenceLock, error) {
	b.chainLock.Lock()
...
// calcSequenceLock计算传递的Tx的相对锁定时间。
// calcSequenceLock computes the relative lock-times for the passed
// transaction. See the exported version, CalcSequenceLock for further details.
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) calcSequenceLock(node *blockNode, tx *drcutil.Tx, utxoView *UtxoViewpoint, mempool bool) (*SequenceLock, error) {
...
// LockTimeToSequence将传递的相对锁时间转换为符合BIP-68的序列号。
...
// getReorganizeNodes发现主链之间的叉点和节点,并返回一个列表,
// 通过块节点需要脱离主链和块的列表节点需要附加到叉点结束(主链分离后返回列表块节点)为了重组链,通过节点是新的主链的结束。
// 如果传递的节点不在侧链上，则列表将为空。
...
// connectBlock处理将传递的节点/块连接到主(best)链的末端。
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) connectBlock(node *blockNode, block *drcutil.Block,
...
// disconnectBlock处理将传递的节点/块从主(best)链的末端断开。
// disconnectBlock handles disconnecting the passed node/block from the end of
// the main (best) chain.
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) disconnectBlock(node *blockNode, block *drcutil.Block, view *UtxoViewpoint) error {
...
// countspentoutput返回传递的块花费的utxos的数量。
// countSpentOutputs returns the number of utxos the passed block spends.
func countSpentOutputs(block *drcutil.Block) int {
...
// reorganizeChain通过断开detachNodes列表中的节点并连接attach列表中的节点来重新组织块链。它期望列表的顺序已经正确，并且与当前最佳链的末端同步。
...
func (b *BlockChain) reorganizeChain(detachNodes, attachNodes *list.List) error {
	...
	detachBlocks := make([]*drcutil.Block, 0, detachNodes.Len())
	detachSpentTxOuts := make([][]SpentTxOut, 0, detachNodes.Len())
	attachBlocks := make([]*drcutil.Block, 0, attachNodes.Len())
...
// connectBestChain处理将传递的块连接到链上，同时根据最可靠的链选择合适的链。
...
// MainChainHasBlock返回带有给定散列的块是否在主链中。
...
// BlockLocatorFromHash返回传递的块散列的块定位器。
...
// LatestBlockLocator返回一个块定位器，用于主链(best)中已知的最新端。
...
// BlockHeightByHash返回带有主链中给定散列的块的高度。
...
// BlockHashByHeight返回主链中给定高度的块的散列。
...
// HeightRange返回给定起始高度和结束高度的块散列范围。它包括起始高度，不包括结束高度。终端高度将限制在当前的主链高度。
...
// HeightToHashRange返回给定起始高度和结束散列的块散列范围，包括两端。
// 哈希值适用于所有高度大于或等于startHeight的endHash祖先块。结束散列必须属于已知有效的块。
...
// IntervalBlockHashes返回endHash的祖先块的哈希值，其中块的高度是interval的正数倍。
...
// locateInventory返回定位器中第一个已知块之后的块的节点，以及到达所提供的停止散列或所提供的条目的最大数量所需的后续节点的数量。
...
// locateBlocks返回定位器中第一个已知块之后的块的散列，直到到达所提供的停止散列，或者达到所提供的最大块散列数。
...
// locateHeaders返回定位器中第一个已知块之后的块的标题，直到到达所提供的stop散列，或者达到所提供的块标题的最大数量。
...
type IndexManager interface {
	...
	// required.
	ConnectBlock(database.Tx, *drcutil.Block, []SpentTxOut) error

	// DisconnectBlock is invoked when a block has been disconnected from
	// the main chain. The set of outputs scripts that were spent within
	// this block is also returned so indexers can clean up the prior index
	// state for this block.
	DisconnectBlock(database.Tx, *drcutil.Block, []SpentTxOut) error
}
