...
// migrateBlockIndex将所有块条目从v1块索引桶迁移到v2桶。
...
// readBlockTree读取旧块索引桶，并构造每个块到其父块和所有子块的映射。
...
// determineblockheight将一个块散列映射到一个子散列切片，并使用它来计算每个块的高度。
...
// determineMainChainBlocks从顶部向下遍历块图，以确定哪些块散列是主链的一部分。
...
//反序列化zeutxoentryv0根据遗留版本0格式将传递的序列化字节片中的utxo条目解码为事务中的输出索引键控制的utxos映射。
...
// upgradeUtxoSetToV2分批将utxo集条目从版本1迁移到2。
...
// maybeUpgradeDbBuckets检查这个包使用的bucket的数据库版本，并执行任何必要的升级，将它们带到最新的版本。
