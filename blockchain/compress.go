...
// serializeSizeVLQ返回根据上面描述的格式将传递的数字序列化为可变长度的数量所需要的字节数。
...
// putVLQ按照上面描述的格式将提供的数字序列化为可变长度的数量，并返回编码值的字节数。
...
// isPubKeyHash返回传递的公钥脚本是否为标准的pay-to-pubkey散列脚本，如果是，则返回它所支付的pubkey散列。
...
// isScriptHash返回传递的公钥脚本是否是标准的付费到脚本散列脚本，如果是，则返回它所支付的脚本散列。
...
// isPubKey返回传递的公钥脚本是否是标准的付费到公钥脚本，该脚本向有效的压缩公钥或未压缩公钥支付费用，如果是，则向序列化的公钥支付费用。
...
// compressedScriptSize返回在使用上面描述的特定于域的压缩算法进行编码时传递的脚本的字节数。
...
// decodeCompressedScriptSize将传递的序列化字节视为压缩脚本，
...
//解压脚本返回根据上面描述的特定于域的压缩算法对传递的压缩脚本进行解压得到的原始脚本。
...
// compressTxOutAmount根据上面描述的特定于域的压缩算法压缩传递的数量。
...
// compressedTxOutSize返回使用上述格式编码时传递的事务输出字段的字节数。
...
//putCompressedTxOut根据域特定的压缩算法压缩传递的数量和脚本，并按照上面描述的格式将它们直接编码到传递的目标字节片中。
...
// decodecomsedtxout解码传递的压缩txout，可能后面跟着其他数据，
