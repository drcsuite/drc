...
func TestHaveBlock(t *testing.T) {
	...
	_, isOrphan, err := chain.ProcessBlock(drcutil.NewBlock(&Block100000),
		BFNone)
	if err != nil {
...
func TestCalcSequenceLock(t *testing.T) {
	...
	// age of 4 blocks.
	targetTx := drcutil.NewTx(&wire.MsgTx{
		TxOut: []*wire.TxOut{{
			PkScript: nil,
			Value:    10,
		}},
	...
	utxoView.AddTxOuts(drcutil.NewTx(unConfTx), 0x7fffffff)
