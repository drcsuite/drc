...
// newHashFromStr将传递的big-endian十六进制字符串转换为一个chain . hash. hash。
...
// LatestCheckpoint返回最近的检查点(不管它是否已知)。
...
//findPreviousCheckpoint查找块链下载部分中已经可用的最近的检查点，并返回相关的块节点。
...
//isNonstandardTransaction确定事务是否包含不属于标准类型的脚本。
// isNonstandardTransaction determines whether a transaction contains any
// scripts which are not one of the standard types.
func isNonstandardTransaction(tx *drcutil.Tx) bool {
...
// IsCheckpointCandidate返回传递的块是否是一个好的检查点候选块。
...
//
// This function is safe for concurrent access.
func (b *BlockChain) IsCheckpointCandidate(block *drcutil.Block) (bool, error) {
	b.chainLock.RLock()
