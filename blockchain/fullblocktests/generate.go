...
// FullBlockTestInstance只允许孤儿院被当作TestInstance来处理。
...
func makeSpendableOutForTx(tx *wire.MsgTx, txOutIndex uint32) spendableOut {
	return spendableOut{
		...
		amount: drcutil.Amount(tx.TxOut[txOutIndex].Value),
	}
}
...
func payToScriptHashScript(redeemScript []byte) []byte {
	redeemScriptHash := drcutil.Hash160(redeemScript)
...
func calcMerkleRoot(txns []*wire.MsgTx) chainhash.Hash {
	...
	utilTxns := make([]*drcutil.Tx, 0, len(txns))
	for _, tx := range txns {
		utilTxns = append(utilTxns, drcutil.NewTx(tx))
	}
...
func additionalCoinbase(amount drcutil.Amount) func(*wire.MsgBlock) {
...
func additionalSpendFee(fee drcutil.Amount) func(*wire.MsgBlock) {
	return func(b *wire.MsgBlock) {
...
// additionalTx返回一个函数，该函数本身接受一个块，并通过添加提供的事务对其进行修改。
...
// tests.
func createSpendTx(spend *spendableOut, fee drcutil.Amount) *wire.MsgTx {
...
func createSpendTxForTx(tx *wire.MsgTx, fee drcutil.Amount) *wire.MsgTx {
...
// updateBlockState手动更新生成器状态，以通过块的旧散列删除所有内部映射引用，并为新的块散列插入新的映射引用。
