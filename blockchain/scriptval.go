...
// sendResult在内部结果通道上发送脚本对验证的结果，同时考虑退出通道。
...
// validateHandler使用要从内部验证通道验证的项，并在内部结果通道上返回验证的结果。
...
// Validate使用多个goroutine验证所有传递的事务输入的脚本。
...
// newTxValidator返回一个新的txValidator实例，用于异步验证事务脚本。
