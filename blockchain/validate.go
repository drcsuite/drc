...
const (
	...
	baseSubsidy = 50 * drcutil.SatoshiPerBitcoin
)
...
func IsCoinBase(tx *drcutil.Tx) bool {
	return IsCoinBaseTx(tx.MsgTx())
}

//SequenceLockActive确定一个事务的序列锁是否已被满足，这意味着给定事务的所有输入都已达到一个足够的高度或时间来满足它们的相对锁时间成熟度。
...
func IsFinalizedTransaction(tx *drcutil.Tx, blockHeight int32, blockTime time.Time) bool {
	msgTx := tx.MsgTx()
...
func CheckTransactionSanity(tx *drcutil.Tx) error {
	...
	for _, txOut := range msgTx.TxOut {
		...
		if totalSatoshi > drcutil.MaxSatoshi {
			str := fmt.Sprintf("total value of all transaction "+
...
// txscript.
func CountSigOps(tx *drcutil.Tx) int {
	msgTx := tx.MsgTx()
...
func CountP2SHSigOps(tx *drcutil.Tx, isCoinBaseTx bool, utxoView *UtxoViewpoint) (int, error) {
...
func ExtractCoinbaseHeight(coinbaseTx *drcutil.Tx) (int32, error) {
...
func checkSerializedHeight(coinbaseTx *drcutil.Tx, wantHeight int32) error {
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) checkBlockContext(block *drcutil.Block, prevNode *blockNode, flags BehaviorFlags) error {
...
func (b *BlockChain) checkBIP0030(node *blockNode, block *drcutil.Block, view *UtxoViewpoint) error {
...
func CheckTransactionInputs(tx *drcutil.Tx, txHeight int32, utxoView *UtxoViewpoint, chainParams *chaincfg.Params) (int64, error) {
	...
	for txInIndex, txIn := range tx.MsgTx().TxIn {
		...
		if originTxSatoshi < 0 {
			str := fmt.Sprintf("transaction output has negative "+
				"value of %v", drcutil.Amount(originTxSatoshi))
			return 0, ruleError(ErrBadTxOutValue, str)
		}
		if originTxSatoshi > drcutil.MaxSatoshi {
			str := fmt.Sprintf("transaction output value of %v is "+
				"higher than max allowed value of %v",
				drcutil.Amount(originTxSatoshi),
				drcutil.MaxSatoshi)
			return 0, ruleError(ErrBadTxOutValue, str)
		}
		...
		if totalSatoshiIn < lastSatoshiIn ||
			totalSatoshiIn > drcutil.MaxSatoshi {
			str := fmt.Sprintf("total value of all transaction "+
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) checkConnectBlock(node *blockNode, block *drcutil.Block, view *UtxoViewpoint, stxos *[]SpentTxOut) error {
