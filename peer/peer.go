...
// goroutine which writes to the network main.
...
func (p *Peer) readMessage(encoding wire.MessageEncoding) (wire.Message, []byte, error) {
	...
	// 使用闭包来记录开销较大的操作，这样只有在日志级别需要时才会运行它们。
...
// inHandler为对等方处理所有传入消息。它必须像goroutine一样运行。
// inHandler handles all incoming messages for the peer.  It must be run as a
// goroutine.
func (p *Peer) inHandler() {

	// 当接收到新消息时，计时器将停止，并在处理后重置计时器。
...
out:
	for atomic.LoadInt32(&p.disconnect) == 0 {
		...
		// 读取一条消息，并在读取完成后立即停止空闲计时器。如果需要，将在下一个迭代中重置计时器。
...
// queueHandler为对等方处理传出数据的排队。它作为各种输入源的muxer运行，因此我们可以确保服务器和对等处理程序不会阻塞我们发送消息。
// 然后将这些数据传递给outHandler进行实际编写。
...
out:
	for {
		...
		case msg := <-p.sendQueue:
			...
			case *wire.MsgPing:
				// 只期望在以后的protoco版本中有一个pong消息。还要建立统计。
			...
			// 此时，消息已成功发送，因此更新最后一次发送时间，通知消息的发送方它已被发送(如果请求)，并向发送队列发送信号以传递下一个排队的消息。
	...
	// 在我们离开之前把所有的等候通道都排干，这样我们就不会留下什么东西等着我们。
	// 我们已经在queueQuit上等待了，因此可以确保不会错过sendQueue上发送的任何内容。
	// Drain any wait channels before we go away so we don't leave something
...
cleanup:
	for {
		...
		case msg := <-p.sendQueue:
			...
			// 无需在sendDoneQueue上发送，因为queueHandler已被等待并已退出。
...
// QueueMessage将传递的比特币消息添加到对等发送队列。这个函数对于并发访问是安全的。
...
// NewOutboundPeer 返回一个新的出站比特币对等点。
