...
const (
	...
	DefaultMinRelayTxFee = drcutil.Amount(1000)
...
// pool and relayed.
func calcMinRequiredTxRelayFee(serializedSize int64, minRelayTxFee drcutil.Amount) int64 {
	...
	if minFee < 0 || minFee > drcutil.MaxSatoshi {
		minFee = drcutil.MaxSatoshi
	}
...
func checkInputsStandard(tx *drcutil.Tx, utxoView *blockchain.UtxoViewpoint) error {
...
func isDust(txOut *wire.TxOut, minRelayTxFee drcutil.Amount) bool {
...
func checkTransactionStandard(tx *drcutil.Tx, height int32,
	medianTimePast time.Time, minRelayTxFee drcutil.Amount,
...
func GetTxVirtualSize(tx *drcutil.Tx) int64 {
	// vSize := (weight(tx) + 3) / 4
